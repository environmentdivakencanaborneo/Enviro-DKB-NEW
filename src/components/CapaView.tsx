...
export default function CapaView({ 
  ...
  const handleDelete = async (id: string) => {
    if (!allowedDelete) return onUnauthorizedAction("Hapus Temuan CAPA");
    try {
                ...
                <div className="p-4 border-t border-[#E6ECE6] flex justify-between shrink-0 bg-white rounded-b-[20px]">
                  {editingId && (
                    <button 
                      onClick={() => {
                        if (!allowedDelete) return onUnauthorizedAction("Hapus Temuan CAPA");
                        setDeleteConfirm({ id: editingId, title: formData.title || 'Temuan' });
                      }}
            ...
            <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
              <div className="bg-white rounded-[20px] shadow-2xl border border-border-custom w-full max-w-md p-6 text-left animate-fade-in">
                ...
                <p className="text-xs text-text-secondary mt-4 leading-relaxed">
                  Apakah Anda yakin ingin menghapus temuan <strong>"{deleteConfirm.title}"</strong> secara permanen? Tindakan ini tidak dapat dibatalkan.
                </p>
                <div className="flex gap-2.5 justify-end mt-6">
                  <button onClick={() => setDeleteConfirm(null)} className="px-4 py-2.5 text-xs font-bold text-text-secondary hover:bg-forest-50 rounded-xl transition-all cursor-pointer">Batal</button>
                  <button onClick={() => handleDelete(deleteConfirm.id)} className="px-4 py-2.5 text-xs font-bold text-white bg-[#A33E3E] hover:bg-[#853030] rounded-xl transition-all shadow-sm cursor-pointer">Hapus Permanen</button>
                </div>
              </div>
            </div>
          </ModalPortal>
